...
The app uses hash-based routing with no library dependencies — `#/` and `#/settings` are the only valid routes, and route changes are just `window.location.hash` assignments. The `<a href="#/settings">` pattern keeps the Back button and accessibility tree working without any framework magic.
