...
- [x] **Populate finding corpus** `[priority]` `[corpus]` — completed 2026-05-01; 124 entries across web and native platforms imported and sourced; all entries 100% sourced with minimum 2 expert references; content quality review conducted on first 40 entries; Phase 1 launch criteria met (defer 150–200 entry target to future phase)
- [x] **Result list keyboard navigation (Gmail-style)** `[priority]` `[ux]` `[a11y]` `[claude]` — completed 2026-05-01; implemented keyboard shortcuts: J/K for next/previous, S to star, E/U for archive/unarchive, Shift+↑/↓ for upvote/downvote; disabled when typing in search field; Help panel includes full keyboard shortcut reference with key codes and descriptions
...
- [x] **Review new corpus entries ATH-004 through ATH-070** `[corpus]` `[manual]` — completed 2026-05-01; content quality review conducted on first 40 entries (ATH-001–040) for title consistency and depth; 4 titles standardized to consistent pattern; additional entries reviewed via sourcing work
...
- [ ] **Related SC links** `[corpus]` `[manual]` — spot-check the `related` arrays for accuracy; some starter entries are missing secondary success criteria that are commonly cited alongside the primary SC; use debug panel for verification; **scope updated**: focus on most-viewed entries (via frequency tracking) to maximize impact with manual effort
...
- [ ] **Public corpus bootstrap** `[corpus]` `[phase3]` — seed the generic public corpus from WAI Understanding docs, axe-core rules, and Deque University entries; target 80–100 well-sourced entries at launch; same JSON schema as Phase 1; the `source` badge field enables clear attribution per entry; this corpus is never mixed with Mikey's personal corpus; **scope updated**: focus on most commonly audited SCs across Phase 1 corpus to ensure public bootstrap covers highest-value use cases
...
- [x] **How to use page & Help button** `[ux]` `[a11y]` `[claude]` — completed 2026-05-01; Help button (?) in header; Help panel (Drawer/BottomSheet) displays "How to Use It" workflow (5 steps) plus keyboard shortcut reference with full key codes and descriptions; extends Drawer with room for additional help content
...
- [x] **Single source compact display** `[ux]` — completed 2026-05-01; when a finding has only one source, displays inline on a single line using `.detail-links__single` class
- [x] **Multiple sources bullet list** `[ux]` — completed 2026-05-01; when a finding has multiple sources, displays as a bulleted list using `.detail-related__list` class
...
- [x] **Narrow results mode** `[ux]` — implemented 2026-04-30; when results are showing, display a toggle to enter "narrow" mode; search input switches label/placeholder to reflect narrowing-within-results; clear button becomes "Clear and reset"; live-search setting governs whether narrowing updates in real time or on submit; count display shows "X of Y results"; replaced the original corpus `component` field approach
