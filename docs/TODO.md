...
- [ ] **Extend agentic AI to remaining providers** `[ai]` `[agent]` `[phase2]` — Agentic mode currently Claude-only (tool use is provider-specific). **Options**: (1) Keep Claude-exclusive agentic mode, improve standard AI assist for other providers (lower effort, user choice of provider remains); (2) Implement OpenAI, Google, Microsoft tool use with provider-specific schemas + routing logic (higher effort, all providers get agentic benefits). **Current**: Claude has full agentic support; OpenAI/Google/Microsoft have standard AI assist (no corpus search). **Recommended**: Defer to Phase 3 if user feedback indicates demand. **Decision**: TBD pending prioritization.
