...
- [x] ~~**Wire OpenAI provider** `[ai]` — implemented in `aiService.js`; `/v1/chat/completions` with Bearer auth and `gpt-4o`; untested pending real API key~~
- [x] ~~**Wire Google Gemini** `[ai]` — implemented in `aiService.js`; `generateContent` endpoint with API key in URL and `gemini-1.5-flash`; untested pending real API key~~
