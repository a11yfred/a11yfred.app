...
- [ ] **Ad tiles in result list** `[ux]` `[design]` `[phase3]` `[manual]` — tile component and admin preview are complete (`SponsoredTile.jsx`, admin ON/OFF toggle, configurable every-N frequency); remaining: connect to a real ad delivery source (Carbon Ads, EthicalAds, or direct-sold), replace placeholder copy, and decide final placement rules before enabling in production; requires ad delivery infrastructure — see ad services item below
