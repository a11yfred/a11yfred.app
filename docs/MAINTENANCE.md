...
- [ ] **Corpus translation coverage** ,  defect descriptions and suggested fix steps in `corpus.json` should have locale-specific overlays; run `npm run translate` whenever entries are added or edited; WCAG SC names and codes (`1.1.1`, `aria-label`, etc.) should remain in English in all locales
